...
use "C:\Users\Regis\Documents\GitHub\teseLegibildiadeCPC\scripts Stata\Atual\Imobilizado\Imobilizado14042020.dta"

keep if LegNE !=. | ADR !=. | LegCPC !=. | RevCPC !=. | CAPIT !=. | COMPLEX !=. | GC !=. | EXT !=. | ADR !=. | Reg_Nreg !=. //manter somente os anos com valores em pelo menos uma variável (são excluidas as linhas que não apresenlnrl valores para as variáveis)
...
**COMPLEX
** Chegou a conclusão que é melhor utilizar sem fazer ajustes no modelo final
...
sum LegNE LegCPC RevCPC lTAM COMPLEX CAPIT GC AUDIT EXT ADR 

sum LegNE LegCPC wlTAM COMPLEX CAPIT WsqEXT
...
** referencia interessante para o teste de curtose e assimetria que é menos sensíveis que os demais testes
...
pwcorr  LegNE LegCPC RevCPC wlTAM COMPLEX CAPIT GC AUDIT EXT ADR, star(0.05) //verifica a correlação (força da associação entre as variáveis) e  ajuda a verificar se há problemas de multicolinearidade (altas correlações)

qui reg LegNE LegCPC RevCPC wlTAM COMPLEX CAPIT GC AUDIT EXT ADR
vif
...
xtserial LegNE LegCPC RevCPC wlTAM COMPLEX CAPIT GC AUDIT EXT ADR, output //roda o teste de woodridge de autocorrelação. 

findit xttest3
qui xtreg LegNE LegCPC RevCPC wlTAM COMPLEX CAPIT GC AUDIT EXT ADR,fe
...
qui xtreg LegNE LegCPC RevCPC wlTAM COMPLEX CAPIT GC AUDIT WsqEXT ADR, re
xttest0
...
xtreg LegNE LegCPC RevCPC wlTAM COMPLEX CAPIT GC AUDIT WsqEXT ADR, fe
...
xtreg LegNE  RevCPC wlTAM COMPLEX CAPIT GC AUDIT WsqEXT ADR, re vce(robust)
