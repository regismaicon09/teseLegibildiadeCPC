...
clear
set more off
use "C:\Users\vafis\OneDrive - ufu.br\Área de Trabalho\endamento.dta"
keep if roe !=. | end !=. | lc !=. | tang !=. | tam !=. | lat !=. //manter somente os anos com valores em pelo menos uma variável (são excluidas as linhas que não apresentam valores para as variáveis)
encode empresas, generate (idempresa) label (empresas) //cria a variável que será utilizada como o indivíduo do painel, transformando-a de string para categórica.
...
histogram end, norm 
kdensity end, norm 
*1)winsorização (técnica para tratar os outliers).
graph box end //muitos outliers *Comentário teórico: O boxplot mostra os outliers.
winsor end, gen(wend) p(0.1) //não tem mais outlier (inicia-se o teste com p(0,05), aumentando de 0,05 em 0,05 até não ter mais outliers).
graph box wend
histogram wend, norm 
kdensity wend, norm
*2)Tratando a normalidade da variável end
...
*2)winsorização (técnica para tratar os outliers).
graph box loglc //muitos outliers *Comentário teórico: O boxplot mostra os outliers.
winsor loglc, gen(wloglc) p(0.05) //não tem mais outlier (inicia-se o teste com p(0,05), aumentando de 0,05 em 0,05 até não ter mais outliers).
graph box wloglc
...
histogram tam, norm 
kdensity tam, norm 
*1)Tratando a normalidade da variável tam
ladder tam //traz as diversas alternativas para transformação da variável 
gladder tam //demonstra em gráficos qual seria a melhor maneira de corrigir a normalidade dos dados 
gen logtam = log(tam) //melhor alternativa seria log-log
*2)winsorização (técnica para tratar os outliers).
graph box logtam //muitos outliers *Comentário teórico: O boxplot mostra os outliers.
winsor logtam, gen(wlogtam) p(0.05) //não tem mais outlier (inicia-se o teste com p(0,05), aumentando de 0,05 em 0,05 até não ter mais outliers).
graph box wlogtam
...
graph box lat //muitos outliers *Comentário teórico: O boxplot mostra os outliers.
winsor lat, gen(wlat) p(0.05) //não tem mais outlier (inicia-se o teste com p(0,05), aumentando de 0,05 em 0,05 até não ter mais outliers).
graph box wlat
histogram wlat, norm 
kdensity wlat, norm
...
* Comentário teórico: Pelos valores dos dois testes pode-se verificar que os termos de erro não apresentam distribuição normal ao nível de significância de 5%, podendo rejeitar a hipótese nula de que os dados possuem distribuição normal.

pwcorr wroe wend wloglc wlogtam wlogtam wlat, star(0.05) //verifica a correlação (força da associação entre as variáveis) e também ajuda a verificar se há problemas de multicolinearidade (altas correlações)

qui reg wroe wend wloglc wlogtam wlogtam wlat
vif
*Comentário teórico: Cada variável não pode apresentar um valor de VIF individualmente maior que 10 e o VIF médio do modelo também não pode ser maior que 10 (HAIR JR. ET AL, 2009). A variável que está causando o problema deve ser retirada do modelo de regressão.
...
xtserial wroe wend wloglc wlogtam wlogtam wlat, output //roda o teste de woodridge de autocorrelação. 
