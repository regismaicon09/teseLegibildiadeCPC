...
*2)winsorização (técnica para tratar os outliers).
graph box lc //muitos outliers *Comentário teórico: O boxplot mostra os outliers.
winsor lc, gen(Wlc) p(0.1) //não tem mais outlier (inicia-se o teste com p(0,05), aumentando de 0,05 em 0,05 até não ter mais outliers).
graph box Wlc
histogram Wlc, norm 
...
tabstat wroe roe end wend lc wlc tang wlnrl lnrl lnat wlnat, s(count min max mean sd cv sk p1 p5 p10 p25 p50 p75 p90 p95 p99)
...
pwcorr wroe wend wlc tang wlnrl wlnat, star(0.05) //verifica a correlação (força da associação entre as variáveis) e lnrlbém ajuda a verificar se há problemas de multicolinearidade (altas correlações)

qui reg wroe wend wlc tang wlnrl wlnat
vif
...
qui xtreg wroe wend wlc tang wlnrl wlnat, re
xttest0
...
xtreg wroe wend wlc tang wlnrl wlnat, fe
...
qui xtreg wroe wend wlc tang wlnrl wlnat, fe
estimates store fe
qui xtreg wroe wend wlc tang wlnrl wlnat, re
estimates store re
...
xtserial wroe wend wlc tang wlnrl wlnat, output //roda o teste de woodridge de autocorrelação. 
...
xtreg wroe wend wlc tang wlnrl wlnat, fe vce(robust)
